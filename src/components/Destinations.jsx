            ...
            return (
              <motion.li key={dest.name} variants={fadeUp} className={isWide ? 'lg:col-span-2' : ''}>
                ...
                >
                  ...
                  />
                  <div className="absolute inset-0 bg-ink/35 transition-opacity duration-500 group-hover:bg-ink/50" aria-hidden="true" />
                  <div className="absolute inset-0 cinematic-overlay" aria-hidden="true" />
                  <div className="relative flex h-full flex-col justify-end p-7">
                    <span className="micro-label mb-2">Asia Speciality</span>
                    <h3 className="font-display text-3xl text-cream">{dest.name}</h3>
                    <p className="text-pretty mt-2 max-w-xl text-sm leading-relaxed text-cream/75">
                      {dest.body}
                    </p>
                    {dest.notice && (
                      <p className="text-xs text-sunset-gold/90 mt-1.5 italic font-mono uppercase tracking-wide">
                        {dest.notice}
                      </p>
                    )}
