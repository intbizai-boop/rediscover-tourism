    ...
    <section id="destinations-preview" className="relative py-24 md:py-32 scroll-mt-16 md:scroll-mt-20 bg-sand/20 border-b border-charcoal/5">
      ...
      <div className="section-shell flex flex-col gap-14">
        ...
        <motion.ul
          ...
          className="grid grid-cols-1 gap-6 sm:grid-cols-2 max-w-5xl mx-auto w-full"
        >
          {FEATURED_DESTINATIONS.map((dest) => (
            <motion.li key={dest.name} variants={fadeUp}>
              <a
                href="#/destination"
                className="group relative block h-72 overflow-hidden rounded-3xl border border-charcoal/10 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-forest focus-visible:ring-offset-2 focus-visible:ring-offset-ivory md:h-80 transition-[transform,box-shadow] duration-500 hover:-translate-y-1 hover:shadow-md"
              >
                <img
                ...
                <div className="relative flex h-full flex-col justify-end p-7 md:p-8">
                  <span className="micro-label mb-2 text-white/90">Speciality Destination</span>
                  <h3 className="font-display text-2xl md:text-3xl text-white">{dest.name}</h3>
                  <p className="text-pretty mt-2 max-w-xl text-sm leading-relaxed text-white/80">
                    {dest.body}
                  </p>
                  <span className="mt-4 inline-flex items-center gap-2 text-sm text-[#b4d2b7] opacity-100 lg:opacity-0 transition-opacity duration-500 lg:group-hover:opacity-100 font-medium">
                    Explore destination
